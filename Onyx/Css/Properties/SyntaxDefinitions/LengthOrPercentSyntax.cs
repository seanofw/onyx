...
{
	internal class LengthOrPercentSyntax<TProp> : Syntax<TProp>
		where TProp : class
	{
		public Func<TProp, Measure, TProp> Constructor { get; }

		public LengthOrPercentSyntax(Func<TProp, Measure, TProp> constructor)
		{
		...
		{
			Measure? measure = ReadLengthOrPercent(lexer);
			if (!measure.HasValue)
				return null;
