...
namespace Onyx.Html.Dom
{
	...
	public class Element : ContainerNode, IAttributeNode
	{
		...
		protected internal virtual void OnAttrChange(string? name, string? oldValue, string? newValue)
		{
			if (name == "id")
			{
				...
				if (!string.IsNullOrEmpty(newValue) && !char.IsLetterOrDigit(newValue[0]) && newValue[0] != '_')
					System.Diagnostics.Debug.WriteLine($"Note: ID '{newValue}' starts with punctuation, which is often a mistake. This may be a bug in your code.");
#endif
...
#if DEBUG
				if (!string.IsNullOrEmpty(newValue) && !char.IsLetterOrDigit(newValue[0]) && newValue[0] != '_')
					System.Diagnostics.Debug.WriteLine($"Note: Classname '{newValue}' starts with punctuation, which is often a mistake. This may be a bug in your code.");
#endif
		...
		/// <summary>
		/// The render box(es) generated by this element.  May be null if the boxes
		...
		/// <summary>
		/// Compute the boxes, if they haven't already been computed, and return them.
		/// </summary>
		public IReadOnlyList<Box> GetComputedBoxes()
		{
			if (_boxes == null)
				ComputeBoxes();
		...
		/// <summary>
		/// Regenerate (from scratch) all boxes produced by this element.
		/// </summary>
